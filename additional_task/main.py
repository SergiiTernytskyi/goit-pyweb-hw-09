...
class QuoteSpider(scrapy.Spider):
    ...
    def parse(self, response, *kwargs):
        for quote_resp in response.xpath("/html//div[@class='quote']"):
            quote = quote_resp.xpath("span[@class='text']/text()").get().strip()
            author = (
                quote_resp.xpath("span/small[@class='author']/text()").get().strip()
            )
            tags = quote_resp.xpath("div[@class='tags']/a/text()").extract()

            yield QuoteItem(quote=quote, author=author, tags=tags)
            yield response.follow(
                url=self.start_urls[0] + quote_resp.xpath("span/a/@href").get(),
